...
axiosAgent.interceptors.request.use(function (config) {
  ...
  if (token) {
    if (!config.headers['x-access-token']) config.headers['x-access-token'] = token;
  }
  return config;
}, function (error) {
...
function handleError(serviceName, err) {
  if (err.response) {
    ...
    console.log("ERROR while calling SERVICE " + serviceName + ": " + JSON.stringify(err.response.data));
